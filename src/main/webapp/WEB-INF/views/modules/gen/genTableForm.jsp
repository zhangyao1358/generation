...
<body>
	<ul class="nav nav-tabs">
        ...
        <li class="active"><a href="${path}/gen/genTable/form?id=${genTable.id}&name=${genTable.name}">业务表${not empty genTable.id?'修改':'添加'}查看</a></li>
        <li><a href="${path}/gen/genScheme/">生成方案列表</a></li>
        <li><a href="${path}/gen/genScheme/form">生成方案添加</a></li>
	</ul>
	<c:choose>
		<c:when test="${empty genTable.name}">
			<form:form id="inputForm" modelAttribute="genTable" action="${path}/gen/genTable/form" method="post" class="form-horizontal">
				<form:hidden path="id"/>
		...
		<c:otherwise>
			<form:form id="inputForm" modelAttribute="genTable" action="${path}/gen/genTable/save" method="post" class="form-horizontal">
				<form:hidden path="id"/>
				<sys:message content="${message}"/>
				<fieldset>
					<legend>基本信息</legend>
